...
function createCube(){
	...
	window.help = "\nHi! Want to learn what Cube Explorer can do?"
...
function createControls(){
	...
	document.addEventListener( 'keydown', function( event ){
		...
		//if(      key === 37 ) cube.twist( 'Y' )//  Left
		//else if( key === 39 ) cube.twist( 'y' )//  Right
		//else if( key === 38 ) cube.twist( 'X' )//  Up
		//else if( key === 40 ) cube.twist( 'x' )//  Down
	...
	document.getElementById( 'container' ).addEventListener( 'click', navOpen )
	document.getElementById( 'container' ).addEventListener( 'touchstart', navOpen )
	...
	document.querySelector( 'nav .closer' ).addEventListener( 'touchstart', navClose )
}
	...
	element.textify = {
		...
		go: function( params ){
			...
			function step(){
				...
				if( params.index > 0 &&
					params.index < params.text.length ){

					if( params.putDown ){

						element.innerHTML = params.text.substr( 0, params.index )
							+ '<span style="visibility:hidden">'
							+ params.text.substr( params.index )
							+ '</span>'
					}
					else {//  Pick up (instead of Put Down)

						element.innerHTML = '<span style="visibility:hidden">'
					...
					if( element.textify.kill === false ) setTimeout( step, params.mpc )
				}
				else {
